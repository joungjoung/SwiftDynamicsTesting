class Solution:
    ...
    def count_trailing_zeroes(self, n: int) -> int:
        if n < 0:
            raise ValueError("n ต้องเป็นจำนวนเต็มไม่ติดลบ")
    ...
    def main_function(self, n: int) -> int:
        return self.count_trailing_zeroes(self.factorial(n))
...
class SolutionV2:
    # AI Round1
    def count_trailing_zeroes_AI1(self, n: int) -> int:
        """
...
class SolutionV3:
    ...
    def count_trailing_zeroes_my1(self, n: int) -> int:
        """
        ...
        loop_count = self.max_num_power_5(n)
        count = 0
        for k in range(1, loop_count + 1):
            count += self.components_5n_power_k(n, k)
...
for n in range(1, 126):
    ...
    Sv2 = SolutionV2()
    Sv3 = SolutionV3()
    print(f"number: {n}, trailing zeros my1: {Sv3.count_trailing_zeroes_my1(n)}, trailing zeroes v2: {Sv2.count_trailing_zeroes_AI1(n)}")
